...
ggplot(mis_resultados, aes(x = Tamanio, y = Tiempo_ms, color = Metodo)) +
  ...
  geom_smooth(method = "loess", span = 0.7, se = FALSE, linewidth = 0.7) + # Líneas más delgadas
  labs(
  ...
  scale_x_continuous(breaks = seq(1000, 10000, by = 1000)) + # Mejores marcas en eje X
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.1))) # Espacio arriba
